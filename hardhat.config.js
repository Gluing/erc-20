...
task("deploy", "Deploy the token to the specified network")
  ...
  .setAction(async taskArgs => {
    const GluingCoin = await ethers.getContractFactory("GluingCoin");

    console.log(`🚀 Deploying token with vault ${taskArgs.vault} and supply ${taskArgs.supply}`);
    const deployed = await GluingCoin.deploy(taskArgs.vault, taskArgs.supply);
...
task("claims", "Create offchain claims for some tokens as defined in a JSON file")
  ...
  .setAction(async taskArgs => {
    ...
    const GluingCoin = await ethers.getContractAt("GluingCoin", taskArgs.token);
    const decimals = await GluingCoin.decimals();

    let createClaim = async (destination, amount, forBlock, nonce) => {
      let hash = await GluingCoin.hashForClaim(signer.address, destination, amount, forBlock, nonce);
...
task("balance", "check the balance of a given user")
  ...
  .setAction(async taskArgs => {
    ...
    const amount = await GluingCoin.balanceOf(taskArgs.addr);
    const ten = ethers.BigNumber.from("10");
